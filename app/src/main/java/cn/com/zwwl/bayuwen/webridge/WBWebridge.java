...
public class WBWebridge {
    ...
    private WBWebridgeImplement mWbWebridgeListener;
    ...
    public WBWebridge(CommonWebView webView, WBWebridgeImplement listener) {
        mWebView = webView;
    ...
    private void jsToNative(JSONObject evalObj) {
        ...
        // 需要执行的native方法参数
        final Object params = evalObj.opt("params");
        if (JSONObject.NULL.equals(evalObj)) {
            evalObj = null;
        }
//         Object parmaObj = evalObj;
        ...
        mHandler.post(new Runnable() {
            ...
            @Override
            public void run() {
                if (!asyncExecuteForCommand(command, params, sequence)) {
                    // 异步方法请求失败，调用同步方法
                    executeForCommand(command, params, sequence);
                }
            }
        });
    }
     ...
     *
     * @param command  本地方法名
     * @param parmaObj 本地方法参数
     * @param sequence 回调的js方法序号
     * @return
     */
    @JavascriptInterface
    private boolean asyncExecuteForCommand(final String command, Object parmaObj, final int sequence) {
        ...
        AsynExecuteCommandListener listener = new AsynExecuteCommandListener() {
            ...
            public void onCallBack(Object... result) {
                ...
                onFetchResult(result[0], command, sequence);
            }
        };
        try {
            if (parmaObj == null) {
                InvokeMethod.invokeMethod(mWbWebridgeListener, command, new Object[]{listener});
            } else {
                InvokeMethod.invokeMethod(mWbWebridgeListener, command, new Object[]{parmaObj, listener});
            }
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    private void executeForCommand(String command, Object parmaObj, int sequence) {
        ...
        try {
            if (parmaObj == null) {
                result = InvokeMethod.invokeMethod(mWbWebridgeListener, command, null);
            } else {
                result = InvokeMethod.invokeMethod(mWbWebridgeListener, command, new Object[]{parmaObj});
            }
        } catch (Exception e) {
            e.printStackTrace();
            callbackJs(sequence, "", COMMAND_ERROR + command);
     ...
     *
     * @param sequence         回调的js方法序号
     * @param nativeReturnJson 回调的js方法参数
     * @param errorMsg         错误信息
     * @return
     */
    @JavascriptInterface
    private void callbackJs(int sequence, String nativeReturnJson, String errorMsg) {
        if (sequence == -1) return;
     ...
     * 本地调用js方法
     *
     * @param jsCommand js方法名
     * @param jsParmas  js方法参数
     * @param command   需要获取js返回值的本地方法
     */
    @JavascriptInterface
    public synchronized void nativeToJs(String jsCommand, Object jsParmas, String command) {
    ...
    private void nativeToJSCallback(JSONObject returnObj) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    if (resultObj == null) {
                        InvokeMethod.invokeMethod(mWbWebridgeListener, command, null);
                    } else {
                        InvokeMethod.invokeMethod(mWbWebridgeListener, command, new Object[]{resultObj});
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
