 ...
 */
public class PullToRefreshScrollView extends ScrollView implements Scrollable {
    ...
    private PullToRefreshScrollViewCallbacks pullToRefreshCallback;
    ...
    private void init(Context context){
        footView = LayoutInflater.from(context).inflate(R.layout.def_loading,null);
    }
...
//                break;
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (getCurrentScrollY() - diffY <= 0) {
                    ...
                    //滑动刷新
                    if (!isRefreshing && getCurrentScrollY() == 0) {
                        if ((ev.getY() - y) < 0) break;//若往下滑动
                        if ((ev.getY() - y) >= zoomViewHeight / 2) {
                            isRefreshing = true;
                        }
                    }
    ...
    public void setIsLoadMore(boolean more) {
        this.isLoadMoreing = more;
    }
    ...
    public void setPullToRefreshCallback(PullToRefreshScrollViewCallbacks listener) {
        pullToRefreshCallback = listener;
    }
        ...
        if (!isLoadMoreing) {
            if (this.getScrollY() + this.getHeight() >= computeVerticalScrollRange()) {
                ...
                post(new Runnable() {
                    @Override
                    public void run() {
                        pullToRefreshCallback.onLoadMore();
