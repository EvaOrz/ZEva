...
public class CustomWeekView extends WeekView {
    ...
    protected void onDrawScheme(Canvas canvas, Calendar calendar, int x) {
        ...
        if (isSelected) {
            ...
            mPointPaint.setColor(0xffdcaa40);
        }

        canvas.drawCircle(x + mItemWidth / 2, mItemHeight - 3 * mPadding, mPointRadius,
                mPointPaint);
    }

    @Override
    protected void onDrawText(Canvas canvas, Calendar calendar, int x, boolean hasScheme, boolean
            isSelected) {
        int cx = x + mItemWidth / 2;
        int cy = mItemHeight / 2;
        ...
        if (hasScheme) {
            canvas.drawCircle(x + mItemWidth - mPadding - mCircleRadius / 2, mPadding +
