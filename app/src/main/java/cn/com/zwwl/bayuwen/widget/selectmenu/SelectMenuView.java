...
public class SelectMenuView extends LinearLayout {
    private static final int TAB_SUBJECT = 1;// 项目
    private static final int TAB_SORT = 2;// 类型
    private static final int TAB_SELECT = 3;// 筛选
    private static final int TAB_GRADE = 4;// 年级
    private static final int TAB_SCHOOL = 5;// 校区
...
//    private SelectHolder mSelectHolder;
    ...
    private ImageView img_sub_grade;
    ...
    private void init() {
        ...
        mPrimaryList.add("全部");
        mPrimaryList.add("海淀魏公村校区");
        mPrimaryList.add("海淀魏公村韦伯时代校区");
        ...
        mSubjectHolder.setOnRightListViewItemSelectedListener(new SubjectHolder
        ...
        mSortHolder.refreshData(projectList, 0);
        mSortHolder.setOnSortInfoSelectedListener(new SortHolder.OnSortInfoSelectedListener() {
            ...
            public void onSortInfoSelected(String info) {
                ...
                dismissPopupWindow();
                mSortText.setText(info);
            }
        });

        mSortHolder2 = new SortHolder(mContext);
        mSortHolder2.refreshData(typeList, 0);
        mSortHolder2.setOnSortInfoSelectedListener(new SortHolder.OnSortInfoSelectedListener() {
            @Override
            public void onSortInfoSelected(String info) {
                ...
                dismissPopupWindow();
                mSubjectText.setText(info);
            }
        });


        mSortHolder3 = new SortHolder(mContext);
        mSortHolder3.refreshData(gradeList, 0);
        mSortHolder3.setOnSortInfoSelectedListener(new SortHolder.OnSortInfoSelectedListener() {
            @Override
            public void onSortInfoSelected(String info) {
...
//
//                dismissPopupWindow();
//                //Toast.makeText(UIUtils.getContext(), gender+" "+type, Toast.LENGTH_SHORT)
// .show();
//            }
//        });
    }
    ...
    protected void onFinishInflate() {
        ...
        img_sub_grade = findViewById(R.id.img_sub_grade);
        ...
        gradeTv.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mOnMenuSelectDataChangedListener != null) {
                    mOnMenuSelectDataChangedListener.onViewClicked(gradeTv);
                }

                handleClickGradeView();
            }
        });
    }
    ...
    private void handleClickSubjectView() {
        ...
        mMainContentLayout.addView(mSubjectHolder.getRootView(), ViewGroup.LayoutParams
                .MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);

        popUpWindow(TAB_SELECT);
    }

    private void handleClickGradeView() {

        mMainContentLayout.removeAllViews();
        mMainContentLayout.addView(mSortHolder3.getRootView(), ViewGroup.LayoutParams
                .MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);

        popUpWindow(TAB_GRADE);
    }

    private void handleClickSortView() {

        mMainContentLayout.removeAllViews();
        mMainContentLayout.addView(mSortHolder.getRootView(), ViewGroup.LayoutParams
                .MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    ...
    private void handleClickSelectView() {
        ...
        mMainContentLayout.removeAllViews();
        mMainContentLayout.addView(mSortHolder2.getRootView(), ViewGroup.LayoutParams
                .MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    ...
    private void setTabExtend(int tab) {
        if (tab == TAB_SUBJECT) {
            mSubjectText.setTextColor(getResources().getColor(R.color.gold));
        ...
        } else if (tab == TAB_GRADE) {
            gradeTv.setTextColor(getResources().getColor(R.color.blue));
            img_sub_grade.setImageResource(R.mipmap.ic_up_blue);
        }
    }

    private void resetTabExtend(int tab) {
        if (tab == TAB_SUBJECT) {
            mSubjectText.setTextColor(getResources().getColor(R.color.gray_dark));
            mSubjectArrowImage.setImageResource(R.mipmap.ic_down);
        } else if (tab == TAB_SORT) {
            mSortText.setTextColor(getResources().getColor(R.color.gray_dark));
            mSortArrowImage.setImageResource(R.mipmap.ic_down);
        } else if (tab == TAB_SELECT) {
            mSelectText.setTextColor(getResources().getColor(R.color.gray_dark));
            mSelectArrowImage.setImageResource(R.mipmap.ic_down);
        } else if (tab == TAB_GRADE) {
            gradeTv.setTextColor(getResources().getColor(R.color.gray_dark));
            img_sub_grade.setImageResource(R.mipmap.ic_down);
        }
    }
